    ...
    const fetchBookInfo = async () => {
      try {
        const response = await axios.get(
          `https://openlibrary.org/search.json?q=${encodeURIComponent(identifier)}`
        );
        if (response.data.docs && response.data.docs.length > 0) {
          ...
          initializeCitationStates(
            response.data.docs.slice(0, numInitialResults).length
          );
        } else {
          setBookInfo([]);
        }
